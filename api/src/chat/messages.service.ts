...
export class MessagesService {
  ...
  async sendMessage(data: Partial<Message>) {
    ...
    console.log('Message created successfully in database:', created);
    ...
    if (messageId && senderId && roomId) {
      ...
      await this.messagesRepo.saveMentions(
        messageId,
        roomId,
        senderId,
        mentions,
  ...
  findByClientMessageId(
    ...
    return this.messagesRepo.findByClientMessageId(
      roomId,
      senderId,
      clientMessageId,
    );
...
function extractMentions(
  content: string,
): Array<{ targetType: 'user' | 'channel'; targetId: string }> {
