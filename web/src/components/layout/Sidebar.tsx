...
import { HiHashtag, HiUserGroup, HiChatBubbleLeftRight, HiPlus, HiMagnifyingGlass, HiChevronDown, HiChevronRight } from 'react-icons/hi2';
      ...
      >
        <div className="relative shrink-0">
          <Avatar
            name={room.name}
            size={32}
            status={isDirect ? status : undefined}
    ...
    <div className="flex flex-col h-full bg-[var(--color-surface)]">
      ...
      <div className="px-4 mb-4">
        <div className="relative group">
          ...
          <input
            className="w-full bg-[var(--color-surface-2)] border border-transparent focus:bg-[var(--color-surface)] focus:border-[var(--color-primary)]/30 focus:ring-4 focus:ring-[var(--color-primary)]/5 rounded-full pl-10 pr-4 py-2 text-sm transition-all duration-300 outline-none placeholder:text-[var(--color-text-muted)]/60"
        ...
        {groupRooms.length > 0 && (
          <section>
            ...
            >
              ...
              <span>Channels</span>
              <div className="flex-1 h-px bg-slate-200 ml-2 opacity-0 group-hover:opacity-100 transition-opacity" />
            </button>
      ...
      <div className="p-4 bg-[var(--color-bg)] border-t border-[var(--color-border)] shadow-[0_-4px_12px_rgba(0,0,0,0.02)]">
        <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-[var(--color-surface-hover)] transition-colors group cursor-pointer">
          <Avatar name={useAuthStore.getState().username || 'Me'} size={36} status="online" className="ring-2 ring-white shadow-sm" />
          <div className="flex-1 min-w-0">
            <div className="text-sm font-bold truncate text-[var(--color-text)] leading-tight">
              {useAuthStore.getState().username || 'Current User'}
            </div>
            <div className="text-[10px] text-[var(--color-text-muted)] font-medium">Online</div>
          </div>
          <Link href="/" className="opacity-0 group-hover:opacity-100 transition-opacity p-2 rounded-lg hover:bg-white text-[var(--color-primary)] shadow-sm">
